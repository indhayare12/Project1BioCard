...
async	function	fetchNewQuote()	{ 
    ...
    quoteCard.classList.add("is-loading"); setButtonsDisabled(true); setStatus("Loading	new	quote...",	""); 
...
function	saveCurrentQuote()	{ 
    ...
    const	alreadySaved	=	favorites.some(function	(fav)	{ 
        ...
        if	(alreadySaved)	{ setStatus("You've	already	saved	this	quote.",	"is-error"); 
            return; }
...
function	renderFavorites()	{ 
    ...
    if	(favorites.length	===	0)	
        { emptyMessage.style.display	=	"block"; return; }
     ...
     favorites.forEach(function	(fav)	{ 
        ...
        removeBtn.addEventListener("click",	function	()	{ removeFavorite(fav.savedAt); }); 
